...
async function buildJs(protectedData) {
  ...
  const builders = [
    ...
    wrapModule({
      ...
      varName: "__m3Theme",
      exports: ["MATERIAL_YOU_THEMES", "generateColorScheme", "applyScheme", "applySourceColor", "restoreSourceColor", "setTheme", "getTheme", "getMaterialTheme", "applyMaterialTheme", "initTheme"]
    }),
    wrapModule({
      fileName: "brand-pill.js",
      varName: "__brandPill",
      exports: ["BRAND_PILL_NAME", "applyBrandPillLogo", "createBrandPillMarkup", "setBrandPillVersion"]
    }),
    wrapModule({
...
async function writeIndex(jsAsset, cssAsset) {
  const htmlIcon = await readFile(path.join(srcRoot, "data", "html-icon.jpg"));
  const bootLogoSrc = `data:image/jpeg;base64,${htmlIcon.toString("base64")}`;
      ...
      <main class="boot-screen" id="bootScreen" aria-live="polite">
        <section class="boot-card" aria-label="${version.name} 正在启动">
          <div class="boot-logo-stage">
            <img class="boot-logo-image" src="${bootLogoSrc}" alt="" />
          </div>
          <div class="boot-copy">
            <h1>${version.name}</h1>
