...
### 核心变化
- 改为基于当前设备实际属性抓取和规则目录生成 `system.prop`，不再依赖厂商模板主链路。
- 安装和重新匹配会自动完成属性抓取、规则匹配和配置生成，减少手动选择步骤。
- 新增统一状态文件 `/data/adb/dex2oat-lock/state.prop`，汇总配置来源、属性摘要、规则匹配、服务、健康和冲突状态。

### WebUI
- 首页改为状态总览，集中展示模块状态、配置来源、属性摘要、最近应用结果和冲突数量。
- 底部导航简化为首页 / 自定义 / 关于，诊断和 GitHub 入口收纳到对应页面。
- 自定义页整合安全 / 谨慎 / 危险模式，保存前展示启用项、变更项、高风险项和覆盖自动匹配提示。
- 配置项开关移到右侧，点击开关只切换状态，点击卡片主体展开说明。
- 危险操作新增风险确认：30 秒等待、算术验证和显式同意，并记住已确认状态。

### 可靠性
- 新增完整性校验，覆盖 WebUI、脚本、规则和关键模块文件，结果会显示在首页和诊断中。
- `service.sh`、`health-check.sh`、`conflict-detect.sh` 统一写入状态，便于排查属性生效、健康检查和冲突问题。
- WebUI 保存配置时同步刷新统一状态，诊断摘要改为卡片化展示。
