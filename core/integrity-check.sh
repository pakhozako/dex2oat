...
hash_file() {
  ...
  if command -v sha256sum >/dev/null 2>&1; then
    case "$TARGET" in
      */module.prop) HASH_VALUE="$(normalize_module_prop "$TARGET" | sha256sum 2>/dev/null | awk '{print $1}')" ;;
      *) HASH_VALUE="$(sha256sum "$TARGET" 2>/dev/null | awk '{print $1}')" ;;
    esac
    [ -n "$HASH_VALUE" ] && { printf '%s' "$HASH_VALUE"; return 0; }
  fi
  printf 'unsupported'
}
...
is_integrity_key_path() {
  case "$1" in
    action.sh|customize.sh|service.sh|uninstall.sh|module.prop|\
core/common.sh|core/conflict-detect.sh|core/health-check.sh|core/input.sh|core/integrity-check.sh|\
core/property.sh|core/prop-lock.sh|core/safety.sh|core/state.sh|core/statectl.sh|\
rules/prop-policy.tsv|rules/rule-props.pack|\
scripts/capture-props.sh|scripts/decode-rules.sh|scripts/generate-props.sh)
      return 0
      ;;
  esac
  return 1
...
scan_baseline() {
  ...
  while IFS='=' read -r REL_PATH EXPECTED_HASH || [ -n "$REL_PATH" ]; do
    ...
    if [ "$ACTUAL_HASH" = "unsupported" ]; then
      HASH_UNSUPPORTED_TOTAL=$((HASH_UNSUPPORTED_TOTAL + 1))
      printf '%s hash-unsupported expected=%s actual=unsupported\n' "$REL_PATH" "$EXPECTED_HASH" >> "$SOURCE_REPORT_TMP"
      continue
    fi
...
check_runtime_file() {
  ...
  case "$KIND" in
    ...
    state)
      ...
      ;;
    tsv)
      if grep -q -E '^[^#].*\t' "$TARGET" 2>/dev/null; then
        printf '%s ok path=%s\n' "$LABEL" "$TARGET" >> "$TMP_REPORT"
      else
        printf '%s invalid-tsv path=%s\n' "$LABEL" "$TARGET" >> "$TMP_REPORT"
        RUNTIME_WARNING_TOTAL=$((RUNTIME_WARNING_TOTAL + 1))
      fi
      ;;
  esac
...
cat "$SOURCE_REPORT_TMP" >> "$TMP_REPORT" 2>/dev/null || true
{
  ...
  printf 'hash_unsupported_total=%s\n' "$HASH_UNSUPPORTED_TOTAL"
  printf '[runtime]\n'
} >> "$TMP_REPORT" 2>/dev/null || STATUS=error
...
elif [ "$HASH_UNSUPPORTED_TOTAL" -gt 0 ] 2>/dev/null; then
  STATUS=ok
  REASON=hash-tool-unavailable
...
if command -v state_update >/dev/null 2>&1; then
  state_update \
    ...
    "integrity.hash_unsupported_total=$HASH_UNSUPPORTED_TOTAL" \
