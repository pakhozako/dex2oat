...
`tools/deploy-cloud-release.py` 只允许操作服务器 `/root/codex-managed/dex2oat-lock` 下的受管发布镜像、API JSON 和首页，不得触碰服务器原有目录或无关服务。
...
- Python 工具不得保存服务器凭据；云端部署必须从环境变量或 SSH key 读取凭据。
...
- 如需云端镜像，`npm run cloud:check` 与 `npm run cloud:deploy` 已确认 `/api/update.json` 和远端 ZIP SHA256。
- 文档已同步更新。
