...
- 当前版本：`v4.1 / 401`
- 项目使用 `package.json` 固定 WebUI 构建与保护工具链，依赖需通过 `node tools\build.js` 或 `npm install` 保持同步。
...
- 启动页 `index.html` 与主界面左上角 Logo 统一使用 `webroot-src/data/logo.jpg`，当前来源是 `D:\dex2oat-work\构建\logo.jpg`，并进入保护构建。
- 启动页歌词保留为：`可以愛的話 不退縮`。
- 顶部胶囊显示 `Dex2oat-Lock`、版本、刷新按钮和重启菜单；刷新按钮位于重启按钮左侧，不在该区域重复显示机型和 Android 版本。
- `设备系统配置摘要` 卡片只保留配置摘要信息，不再显示机型或设备 ID。
- 自定义工作台不得显示“自定义已确认”“危险模式”“待确认”等噪声文案。
- 卡片透明度设置控制的是前景卡片透明度，不是背景图透明度；取值应覆盖 0-100。
- 模糊强度设置同样应覆盖 0-100。
- 安装历史应使用弹窗展示，不跳转到独立页面。
- 首页功能中心保持 2×2 手机端布局：诊断、安装历史、system.prop、重匹配；不要再添加重复的自定义跳转或第二个刷新入口。
- 关于页保留可关闭的模块通信开关和手动规则证据上传，不展示公开用户面板或公开统计入口。
